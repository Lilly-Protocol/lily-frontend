    ...
    <nav aria-label={ariaLabel}>
      <ul className="grid gap-2">
        {routes.map((route) => {
          ...
          const isActive = !isDynamicPlaceholder && isRouteActive(pathname, route.path);
            ...
            <li key={route.id} className="sm:w-64 sm:flex-none">
              {isDynamicPlaceholder ? (
                <div className="flex items-center justify-between rounded-2xl border border-dashed border-(--color-line) bg-(--color-panel-muted) px-4 py-3 text-sm">
                  ...
                  <span className="font-mono text-xs text-(--color-muted)">{route.path}</span>
                </div>
              ) : (
                <Link
                  className={`flex items-center justify-between rounded-2xl border bg-(--color-panel-muted) px-4 py-3 text-sm ${
                    isActive
